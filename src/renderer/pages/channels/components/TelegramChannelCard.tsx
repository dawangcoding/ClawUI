...
import type { TelegramStatus } from '../../../../shared/types/gateway-protocol'
...
export default function TelegramChannelCard(props: ChannelCardProps) {
   const { channelKey, label, description, accounts, channelStatus, loading, onRefresh, accent, letter, enabled } =
      props
   const status = channelStatus as TelegramStatus | null
      ...
      >
         <div className={styles.cardHeader}>
            <div className={styles.cardTitleArea}>
               ...
               {botUsername && <span className={styles.botTag}>@{botUsername}</span>}
            </div>
            ...
            {accounts.length >= 2 ? (
               <ChannelAccountCards
                  ...
                  extraFields={(a) => {
                     ...
                     const probe = a.probe as TelegramStatus['probe'] | undefined
                     if (probe?.bot?.username) {
                        extra.push({ label: 'Bot', value: `@${probe.bot.username}` })
                     }
                     return extra
                  }}
               />
